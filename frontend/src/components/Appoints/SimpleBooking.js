...
import { getAllDoctors, getAllPatients, bookAppointment } from '../../services/api'
    ...
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
        ...
        <div>
          ...
          <div style={{ maxHeight: '200px', overflowY: 'auto', border: '1px solid #ddd', padding: '10px' }}>
            {doctors.map((doctor) => (
              <div key={doctor.medicalId} style={{ 
                padding: '8px', 
                border: '1px solid #eee', 
                marginBottom: '5px',
                backgroundColor: booking.doctorMedicalId === doctor.medicalId ? '#e8f5e8' : 'white'
              }}>
                <strong>{doctor.username}</strong><br/>
                <small>Medical ID: {doctor.medicalId}</small><br/>
                {doctor.specialty && <small>Specialty: {doctor.specialty}</small>}
                <button 
                  onClick={() => setBooking({...booking, doctorMedicalId: doctor.medicalId})}
                  style={{ 
                    marginLeft: '10px', 
                    padding: '2px 8px',
        ...
        <div>
          ...
          <div style={{ maxHeight: '200px', overflowY: 'auto', border: '1px solid #ddd', padding: '10px' }}>
            {patients.map((patient) => (
              <div key={patient.medicalId} style={{ 
                padding: '8px', 
                border: '1px solid #eee', 
                marginBottom: '5px',
                backgroundColor: booking.patientMedicalId === patient.medicalId ? '#e8f5e8' : 'white'
              }}>
                <strong>{patient.username}</strong><br/>
                <small>Medical ID: {patient.medicalId}</small><br/>
                <small>Email: {patient.email}</small>
                <button 
                  onClick={() => setBooking({...booking, patientMedicalId: patient.medicalId})}
                  style={{ 
                    marginLeft: '10px', 
                    padding: '2px 8px',
      ...
      <div style={{ marginTop: '20px', padding: '10px', backgroundColor: '#e9ecef', borderRadius: '4px' }}>
        ...
        <ul>
          ...
          <li>Select a patient from the list (uses Medical ID like "PAT001")</li>
